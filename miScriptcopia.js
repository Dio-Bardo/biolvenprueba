...
const secretKey = 'd1UBSbIzcvnsU6e'; // Clave secreta para firmar el token JWT
...
function verifyTokenMiddleware(req, res, next) {
  const authToken = req.cookies.authToken; // Obtener el token de la cookie

  // Verificar si el token está presente y es válido
  if (authToken) {
    const decodedToken = verifyToken(authToken);
    if (decodedToken) {
      // El token es válido, puedes acceder a la información del usuario desde el token decodificado
      req.user = { userId: decodedToken.userId }; // Guardar la información del usuario en la solicitud
...
app.get('/userDataCalendario', async (req, res) => {
  try {
    const emails = userEmailInput;
    if (!emails) {
      throw new Error('No se ha iniciado sesión o no se ha proporcionado un email.');
    }
...
<body class="sub_page">
  ...
  <body>
    ...
    <section class="about_section layout_padding">
      ...
      <h2>Subir Archivos <span id="flecha" onclick="toggleMenu()">&#9660;</span></h2>
      ...
      <div id="archivoMenu" style="display: none;">
        ...
        <ul id="archivoList">
          ...
          <li><button id="limpiarSliderBtn" onclick="limpiarSlider()">Limpiar Slider</button></li>
          <li><button onclick="subirAlSlider()">Subir al Slider</button></li>
        </ul>

      <script>
      function toggleMenu() {
        const menu = document.getElementById('archivoMenu');
        const flecha = document.getElementById('flecha');
  
        if (menu.style.display === 'none') {
          menu.style.display = 'block';
          flecha.innerHTML = '&#9650;'; // Cambiar la flecha hacia arriba
        } else {
          menu.style.display = 'none';
          flecha.innerHTML = '&#9660;'; // Cambiar la flecha hacia abajo
        }
      }
      </script>
